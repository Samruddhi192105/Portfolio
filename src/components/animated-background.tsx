...
type SplineColor = {
  ...
  setRGBA?: (c: { r: number; g: number; b: number; a: number }) => void;
...
type SplineMat = {
  ...
  needsUpdate?: boolean;
  map?: unknown;
  uniforms?: Record<string, { value?: SplineColor }>;
};
...
type SplineMesh = {
  ...
  traverse?: (cb: (child: SplineMesh) => void) => void;
  traverseEntity?: (cb: (child: SplineMesh) => void) => void;
};

const KEYCAP_MESH_NAMES = new Set(["keycap", "keycap-desktop", "keycap-mobile"]);
const LEGEND_SCALED = new WeakSet<object>();
const BAKED_LABELS = ["Vuejs", "Nginx", "Firebase", "SQL"] as const;

function walkSplineGraph(root: SplineMesh | undefined, visit: (node: SplineMesh) => void) {
  if (!root) return;
  const seen = new Set<SplineMesh>();
  const walk = (node: SplineMesh | undefined) => {
...
function materialsOf(node: SplineMesh): SplineMat[] {
  const raw = node.material ?? node.object?.material;
  if (!raw) return [];
  return (Array.isArray(raw) ? raw : [raw]).filter(Boolean) as SplineMat[];
}

function setSplineColor(target: SplineColor | undefined, hex: number) {
  if (!target) return;
  ...
  try {
    target.setRGBA?.({ r, g, b, a: 1 });
  } catch {
    /* ignore */
  }
  if (typeof target.r === "number") {
    target.r = r;
    target.g = g;
    target.b = b;
  }
  if (target.value && target.value !== target) setSplineColor(target.value, hex);
}

function paintMaterial(mat: SplineMat, hex: number) {
  setSplineColor(mat.color, hex);
  setSplineColor(mat.uniforms?.color?.value, hex);
  setSplineColor(mat.uniforms?.diffuse?.value, hex);
  mat.needsUpdate = true;
}

/** Opposite of the icon so the logo reads: dark mark → light key, warm mark → cool key. */
const KEYCAP_CONTRAST: Record<string, number> = {
  js: 0x4338ca,
  ts: 0xea580c,
  html: 0x1d4ed8,
  css: 0xea580c,
  ...
  vue: 0xf59e0b,
  nextjs: 0xf8fafc,
  tailwind: 0x7c3aed, // purple
  nodejs: 0x9f1239,
  express: 0xf8fafc,
  postgres: 0xd97706,
  mongodb: 0xfacc15, // yellow
  git: 0x2563eb, // blue
  Git: 0x2563eb,
  github: 0xf8fafc,
  prettier: 0x3776ab, // Python blue
  npm: 0x0f766e,
  firebase: 0xf8fafc,
  wordpress: 0xea580c,
  linux: 0xf8fafc, // Kafka — light key so the mark reads
  docker: 0xc2410c,
  nginx: 0x9d174d,
  aws: 0xf8fafc,
  vim: 0xf8fafc,
...
function applyKeycapContrast(app: Application) {
  const scene = (
    app as unknown as {
      _scene?: {
        getObjectByName?: (name: string) => SplineMesh | undefined;
        traverseEntity?: (cb: (child: SplineMesh) => void) => void;
      };
    }
  )._scene;
  ...
  const paintGroup = (root: SplineMesh, hex: number) => {
    walkSplineGraph(root, (child) => {
      const name = child.name ?? "";
      if (name === "legend") return;
      if (!KEYCAP_MESH_NAMES.has(name) && name !== "") return;
      for (const mat of materialsOf(child)) {
        if (mat.map) continue;
        paintMaterial(mat, hex);
      }
    });
  };

  for (const [name, hex] of Object.entries(KEYCAP_CONTRAST)) {
    try {
      const obj = scene.getObjectByName?.(name);
      if (obj) paintGroup(obj, hex);
    } catch {
      /* ignore */
    }
  }
  ...
  try {
    scene.traverseEntity?.((child) => {
      const hex = KEYCAP_CONTRAST[child.name ?? ""];
      if (hex) paintGroup(child, hex);
    });
  } catch {
    /* ignore */
  }
...
function punchLegendMaterial(mat: SplineMat | undefined) {
  if (!mat) return;
  try {
    ...
    mat.needsUpdate = true;
    paintMaterial(mat, 0xffffff);
  } catch {
    /* Spline materials are not full Three.js materials */
  }
}

/** Legend is often a GROUP, not the mesh — punch every mesh under it. */
function enableLogoTransparency(app: Application) {
  const scene = (
    app as unknown as {
      _scene?: SplineMesh;
    }
  ...
  walkSplineGraph(scene, (child) => {
    if (child?.name !== "legend") return;
    walkSplineGraph(child, (node) => {
      for (const mat of materialsOf(node)) punchLegendMaterial(mat);
    });
    const scale = child.scale ?? child.object?.scale;
    if (scale && typeof scale.x === "number" && !LEGEND_SCALED.has(child)) {
      scale.x *= 1.65;
      scale.y *= 1.65;
      LEGEND_SCALED.add(child);
    }
  });
}
...
const KeyboardScene = ({ maxDpr }: { maxDpr: number }) => {
  ...
  const updateKeyboardTransform = async () => {
    ...
    hideBakedLabels(splineApp);
    enableLogoTransparency(splineApp);
    applyKeycapContrast(splineApp);
    ...
    <Suspense fallback={<div>Loading...</div>}>
      <Spline
        ...
        onLoad={(app: Application) => {
          hideBakedLabels(app);
          enableLogoTransparency(app);
          applyKeycapContrast(app);
          setSplineApp(app);
          bypassLoading();
        }}
        scene="/kbd-aj-v5.spline"
      />
    </Suspense>
  );
};
